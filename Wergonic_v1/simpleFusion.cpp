                                     ...
                                     AngleUnit angleUnit)
{
    ...
    pitchFromAccel = atan2(-accelerometer.x, sqrt(pow(accelerometer.y, 2) +
                                                  pow(accelerometer.z, 2)));
    rollFromAccel = atan2(accelerometer.y, sqrt(pow(accelerometer.x, 2) +
                                                pow(accelerometer.z, 2)));

    // Complimentary Filter
    // _pitch = (_pitchGyroFavoring) *
    //              (_pitch + (gyroscope.y * (1.00 / _filterUpdateRate))) +
    //          (1.00 - _pitchGyroFavoring) * (pitchFromAccel);
    // _roll = (_rollGyroFavoring) *
    //             (_roll + (gyroscope.x * (1.00 / _filterUpdateRate))) +
